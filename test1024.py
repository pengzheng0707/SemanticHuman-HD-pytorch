    ...
    with torch.no_grad():
        ...
        if 'gen_interp' in type:
            ...
            for _i in trange(len(zs1)):
                ...
                # c1 = torch.load('result/111/00000_c.pt')
        ...
        # c_map = torch.from_numpy(dataset[10]).pin_memory().to(device).unsqueeze(0)
        # new_c_map = torch.from_numpy(dataset[5636]).pin_memory().to(device).unsqueeze(0)
        ##############
        # c_map[:,111:] = new_c_map[:,111:] 
        ##############
        ...
        # c_first = torch.from_numpy(dataset[4085]).pin_memory().to(device).unsqueeze(0)
        # if True:
        #     c_map = torch.load('c.pt')
        #     # z_map = torch.load('z.pt')
        #     z_map= G.mapping(torch.load('result/1024test2/00014_z.pt'), c_map, truncation_psi=1)
        for _i in trange(len(zs)):
            ...
            if 'gen_video' in type:
                ...
                save_path = f'{output_path}/{_i:05}.mp4'
                z = torch.load('result/video/00040_z.pt')
                c = torch.load('result/video/00040_c.pt')
                gen_video(G, z, c, truncation,res, is_img, is_img_raw, is_normal, is_mesh, save_path, angles)
                
            if 'gen_anim' in type:
                z = torch.load('result/animation3/00016_anim_z.pt')
                c = torch.load('result/animation3/00016_anim_c.pt')
