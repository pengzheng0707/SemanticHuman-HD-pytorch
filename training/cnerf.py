...
class Generator(nn.Module):
    ...
    def forward(self, styles, cam_poses, focals, near=0.88, far=1.12, return_latents=False,
                ...
                project_noise=False, mesh_path=None, mix_styles=True, styles_global=None, semantics=None,new_styles=None):
        ...
        with torch.set_grad_enabled(self.is_train and self.train_renderer):
            ...
            # new_styles = self.style(new_styles)

            # # new_styles = 0.7 * new_styles + 0.3 * truncation_latent[0]
            # latent[:,15:20,:] = new_styles.unsqueeze(1).repeat(1, 5, 1)
...
class VolumeRenderDiscriminator(nn.Module):
    ...
    def forward(self, img, seg):
        ...
        out1 = self.convs_img(img)
        out2 = self.convs_seg(seg)
        out = out1 + out2
...
class ComponentDualBranchDiscriminator(nn.Module):
    def __init__(self):
        ...
        self.class_linear = nn.Sequential(
            EqualLinear(self.channels[4] * 2 * 2, self.channels[4], activation='fused_lrelu'),
            EqualLinear(self.channels[4], n_wcode),
        )
    ...
    def forward(self, image):
        img = image['image']
        full_seg = image['image_seg']
        
        semantic_label = torch.randint(0, 9, [len(img)]).to(img.device)
        seg = torch.zeros(img.size(0), 1, img.size(2), img.size(3)).to(img.device)
        for j,label in enumerate(semantic_label):
            seg[j, 0] = full_seg[j, label]
        ...
        out2 = F.cross_entropy(out2, semantic_label)
        return out1, out2
...
class Discriminator(nn.Module):
    def __init__(self, blur_kernel=[1, 3, 3, 1]):
        ...
        ### seg
        convs_seg = [ConvLayer(9, channels[init_size], 1)]
