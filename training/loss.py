...
class AG3DLoss(Loss):
    ...
    def run_G(self, z, c, swapping_prob, neural_rendering_resolution, update_emas=False):
        ...
        gen_output['image_face'] = crop_face(gen_output['image_raw'], c, size=gen_output['image_raw'].shape[-2]//8)
    ...
    def run_D(self, img, c, blur_sigma=0, blur_sigma_raw=0, cur_tick=0, update_emas=False):
        ...
        # logits['face'] = logits['face']*0.0
        # if (cur_tick % 3) :
        #     logits['full'] = logits['full']*0.005
        # else:
        #     logits['full'] = logits['full']*0.0
        # logits['full'] = logits['full']*0.0005
        # logits['lr'] = logits['lr']*0.0
        return logits

    def accumulate_gradients(self, phase, real_img, real_seg, real_normal, real_c, gen_z, gen_c, gain, cur_nimg, cur_tick):
        ...
        real_img = {'image': real_img,
                    ...
                    'image_face': crop_face(real_img_raw, real_c, size=real_img_raw.shape[-2]//8),
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                gen_logits = self.run_D(gen_img, gen_c, blur_sigma=blur_sigma, cur_tick=cur_tick)
                total_logits = 0
        ...
        if phase in ['Dmain', 'Dboth']:
            with torch.autograd.profiler.record_function('Dgen_forward'):
                ...
                gen_logits = self.run_D(gen_img, gen_c, blur_sigma=blur_sigma, cur_tick=cur_tick, update_emas=True)
                total_logits = 0
                for name in gen_logits:
                    training_stats.report('Loss/scores/fake/%s'%name, gen_logits[name])
                    training_stats.report('Loss/signs/fake/%s'%name, gen_logits[name].sign())
        ...
        if phase in ['Dmain', 'Dreg', 'Dboth']:
            ...
            with torch.autograd.profiler.record_function(name + '_forward'):
                ...
                real_logits = self.run_D(real_img_tmp, real_c, blur_sigma=blur_sigma, cur_tick=cur_tick)
                    ...
                    if self.dual_discrimination:
                        with torch.autograd.profiler.record_function('r1_grads'), conv2d_gradfix.no_weight_gradients():
                            ...
                            for i in range(len(r1_grads)):
                                if i == 1:
                                    r1_penalty += r1_grads[i].square().sum([1,2,3])*100
                                elif i == 2:
                                    r1_penalty += r1_grads[i].square().sum([1,2,3])*10
                                else:
