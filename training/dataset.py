...
#     # label_fusion.append(label[3]+label[5]+label[6]) # pants
...
#     label_fusion.append(label[13]+label[15]+label[14]) # body
#     label_fusion.append(np.sum(label[1:13],axis=0)+np.sum(label[16:],axis=0)) # other
...
def png_to_mask(mask):
    ...
    label_fusion = []
    # label_fusion.append(label[0]) # back
    label_fusion.append(label[15]+label[14]+label[13]) # body
    # label_fusion.append(label[13]) # hair
    label_fusion.append(label[1]+label[4]+label[21]) # top
    label_fusion.append(label[2]) # outer
    # label_fusion.append(label[2]) # outer
    ...
    # label_fusion.append(label[10]) # belt
    # label_fusion.append(label[18]) # sock
    label_fusion.append(label[11]+label[18]) # shoe
    # label_fusion.append(label[12]) # bag
    label_fusion.append(label[7]+label[9]+label[10]+label[12]+label[16]+label[17]+label[19]+label[20]+label[22]+label[23]+label[8]) # other
...
class DeepFashionDataset(Dataset):
    ...
    def _open_file(self, fname):
        if self._type == 'dir':
            return open(os.path.join(self._path, 'images', fname), 'rb')
        if self._type == 'zip':
            return self._get_zipfile().open(fname, 'r')
        return None

    def _open_normal(self, fname):
        return open(os.path.join(self._path, 'normal', fname), 'rb')

    def _open_seg(self, fname):
        return open(os.path.join(self._path, 'segm', fname.replace('.png', '_segm.png')), 'rb')
    ...
    def _load_raw_image(self, raw_idx):
        ...
        image = np.pad(image,((0,0),(256,256),(0,0)),'constant',constant_values=255)
        ...
        seg = np.pad(seg,((0,0),(self._resolution//4,self._resolution//4)),'constant',constant_values=0)
        seg = png_to_mask(seg)
        ...
        normal = np.pad(normal,((0,0),(256,256),(0,0)),'constant',constant_values=255)
    ...
    #     if self._resolution and image.shape[0] != self._resolution:
    #         image = cv2.resize(image, (self._resolution, self._resolutio))
    #     image = image.transpose(2, 0, 1) # HWC => CHW
    #     return image
    ...
    def _load_raw_labels(self):
        smpl_list = np.load(file=open('label.npy', 'rb'))        
        return smpl_list
        # return smpl_list[:,:111]