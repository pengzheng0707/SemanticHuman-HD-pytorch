...
class SNARFDeformer():
    ...
    def __call__(self, pts, model, eval_mode=True, render_skinning=False, is_normal=True, semantic_mask=None):
        ...
        # print(torch.max(pts_cano_all[...,0]))
        # print(torch.min(pts_cano_all[...,0]))
        # print(torch.max(pts_cano_all[...,1]))
        # print(torch.min(pts_cano_all[...,1]))
        # print(torch.max(pts_cano_all[...,2]))
        # print(torch.min(pts_cano_all[...,2]))

        valid = others["valid_ids"].reshape(b, n, -1)
        if semantic_mask is not None:
            rgb_cano, sigma_cano, grad_pred_cano = model(pts_cano_all, valid, semantic_mask)
        else:
            rgb_cano, sigma_cano, grad_pred_cano = model(pts_cano_all, valid)

        # _, idx = torch.max(torch.sum(sigma_cano,dim=-1,keepdim=True).squeeze(-1), dim=-1)
        sigma_cano, idx = torch.max(sigma_cano.squeeze(-1), dim=-1)
        # sdf_cano = torch.gather(sdf_cano, 2, idx[:, :, None, None].repeat(1,1,1,sdf_cano.shape[-1]))
        ...
        ret_valid = torch.gather(valid[:,:,:,None], 2, idx[:, :, None, None])
        if is_normal:
            # grad_cano = torch.gather(grad_cano, 2, idx[:, :, None, None].repeat(1,1,1,grad_cano.shape[-1]))
            grad_pred_cano = torch.gather(grad_pred_cano, 2, idx[:, :, None, None].repeat(1,1,1,grad_pred_cano.shape[-1]))
            grad = torch.zeros_like(grad_pred_cano)
            with torch.enable_grad():
                for i in range(b):
                    pts_ret = pts_cano[i][ret_valid[i,...,0]]
                    pts_ret.requires_grad = True
                    smpl_sdf = self.deformer.query_sdf(pts_ret[None].clone())[0] #(N,1)
                    smpl_grad = sdf_gradient(pts_ret, smpl_sdf) #(N,1)
                    grad_valid = smpl_grad.detach() + grad_pred_cano[i][ret_valid[i,...,0]]
                    grad_valid = self.deformer.skinning_normal(pts_ret, grad_valid, self.tfs)
                    grad[i][ret_valid[i,...,0]] = grad_valid
        ...
        # return rgb_cano, sigma_cano, grad, torch.cat([pts_cano, ret_valid], -1)
        return rgb_cano, sigma_cano, grad, grad_pred_cano, torch.cat([pts_cano, ret_valid], -1)
        # return rgb_cano, sigma_cano.unsqueeze(-1), grad, grad_pred_cano, torch.cat([pts_cano, ret_valid], -1)
