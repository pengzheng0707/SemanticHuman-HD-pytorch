...
class VolumeRenderDiscriminator(nn.Module):
    def __init__(self):
        ...
        ### seg
        convs_seg = [VolumeRenderDiscConv2d(n_wcode, channels[init_size], 1, activate=True)]
        log_size = int(math.log(init_size, 2))
        in_channel = channels[init_size]
...
class ComponentDualBranchDiscriminator(nn.Module):
    ...
    def forward(self, gen_imgs, seg):
        semantic_label = torch.randint(0, 9, [len(gen_imgs)]).to(gen_imgs.device)
