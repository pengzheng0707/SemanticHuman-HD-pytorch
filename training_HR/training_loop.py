...
def training_loop(
    ...
    if resume_pkl is not None:
        ...
        G_kwargs['sigmoid_beta'] = resume_data['G'].renderer.sigmoid_beta
    ...
    if (resume_pkl is not None) and (rank == 0):
        ...
        for name, module in [('G', G), ('G_ema', G_ema)]:
            misc.copy_params_and_buffers(resume_data['G_ema'], module, is_load_noise, require_all=False )
            # misc.copy_params_and_buffers(resume_data[name], module, is_load_noise, require_all=False )
        misc.copy_params_and_buffers(resume_data['G_ema'].decoder, G.decoder_HR, is_load_noise, require_all=False )
        misc.copy_params_and_buffers(resume_data['G_ema'].decoder, G_ema.decoder_HR, is_load_noise, require_all=False )
        # misc.copy_params_and_buffers(resume_data['D'].D_image, D.D_lr, is_load_noise, require_all=False )
        # misc.copy_params_and_buffers(resume_data['D'].D_image, D.D_sr, is_load_noise, require_all=False )
        # misc.copy_params_and_buffers(resume_data['D'].D_seg, D.D_seg, is_load_noise, require_all=False )
        misc.copy_params_and_buffers(resume_data['D'].D_face_image, D.D_face_image, is_load_noise, require_all=False )
        misc.copy_params_and_buffers(resume_data['D'].D_normal, D.D_normal, is_load_noise, require_all=False )
        misc.copy_params_and_buffers(resume_data['D'].D_face_normal, D.D_face_normal, is_load_noise, require_all=False )
    ...
    if rank == 0:
        ...
        # # seg_real = save_image_grid(segs[:,:1], os.path.join(run_dir, 'reals_mask.png'), drange=[-1,1], grid_size=grid_size)
        # del seg_real
        # del normal_real
        # del img_real
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            ...
            if phase.name in ['Dmain','Dreg']:
                phase.module.requires_grad_(True)
            else:
                phase.module.triplane_SR.requires_grad_(True)
                phase.module.decoder_HR.requires_grad_(True)
                phase.module.renderer_HR.requires_grad_(True)
            ...
            for real_img, real_seg, real_normal, real_c, gen_z, gen_c in zip(phase_real_img, phase_real_seg, phase_real_normal, phase_real_c, phase_gen_z, phase_gen_c):
                loss.accumulate_gradients(phase=phase.name, real_img=real_img, real_seg=real_seg, real_normal=real_normal, real_c=real_c, gen_z=gen_z, gen_c=gen_c, gain=phase.interval, cur_nimg=cur_nimg)
            if phase.name in ['Dmain','Dreg']:
                phase.module.requires_grad_(False)
            else:
                phase.module.triplane_SR.requires_grad_(False)
                phase.module.decoder_HR.requires_grad_(False)
                phase.module.renderer_HR.requires_grad_(False)
            # phase.module.requires_grad_(False)
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_nimg % (image_snapshot_ticks*100) == 0 or (cur_nimg % (image_snapshot_ticks*10) == 0 and cur_nimg < 1000) or (cur_nimg<2)):
        # if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_nimg % (image_snapshot_ticks*100) == 0 or (cur_nimg % image_snapshot_ticks == 0 and cur_nimg < 1000) or (cur_nimg < 10)):
        # if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_nimg % (image_snapshot_ticks*100) == 0 or (cur_nimg < 1000)):
            with torch.no_grad():
            ...
            if is_normal:
                ...
                normal_raw = torch.cat([o['normal_raw'].cpu() for o in out]).numpy()
            images_depth = -torch.cat([o['image_depth'].cpu() for o in out]).numpy()
            images_seg = torch.cat([o['image_seg'].cpu() for o in out]).numpy()
            seg_raw = torch.cat([o['seg_raw'].cpu() for o in out]).numpy()
            image_resize = torch.cat([filtered_resizing(o['image'], size=o['image_raw'].shape[-2], f='antialiased').cpu() for o in out]).numpy()
            seg_resize = torch.cat([filtered_resizing(o['image_seg'], size=o['seg_raw'].shape[-2], f='antialiased').cpu() for o in out]).numpy()

            # image_up_resize = torch.cat([filtered_resizing(o['image_raw'], size=o['image'].shape[-2], f='antialiased').cpu() for o in out]).numpy()
            # image_up_filter = torch.cat([img_filter(o['image_raw']).cpu() for o in out]).numpy()
            ...
            seg_raw = save_seg_grid(seg_raw, os.path.join(run_dir, f'fakes{cur_nimg:09d}_seg_raw.png'), drange=[-1,1], grid_size=grid_size)
            ...
            if is_normal:
                ...
                normal_raw = save_image_grid(normal_raw, os.path.join(run_dir, f'fakes{cur_nimg:09d}_normal_raw.png'), drange=[-1,1], grid_size=grid_size)
            image_resize = save_image_grid(image_resize, os.path.join(run_dir, f'fakes{cur_nimg:09d}_image_resize.png'), drange=[-1,1], grid_size=grid_size)
            seg_resize = save_seg_grid(seg_resize, os.path.join(run_dir, f'fakes{cur_nimg:09d}_seg_resize.png'), drange=[-1,1], grid_size=grid_size)

            # image_up_resize = save_image_grid(image_up_resize, os.path.join(run_dir, f'fakes{cur_nimg:09d}_image_up_resize.png'), drange=[-1,1], grid_size=grid_size)
            # image_up_filter = save_image_grid(image_up_filter, os.path.join(run_dir, f'fakes{cur_nimg:09d}_image_up_filter.png'), drange=[-1,1], grid_size=grid_size)
