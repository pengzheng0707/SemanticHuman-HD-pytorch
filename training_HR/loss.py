    ...
    def run_D(self, img, c, blur_sigma=0, blur_sigma_raw=0, update_emas=False):
        ...
        if blur_size > 0:
            with torch.autograd.profiler.record_function('blur'):
                f = torch.arange(-blur_size, blur_size + 1, device=img['image_raw'].device).div(blur_sigma).square().neg().exp2()
                img['image_raw'] = upfirdn2d.filter2d(img['image_raw'], f / f.sum())
                if self.is_normal:
                    img['image_normal'] = upfirdn2d.filter2d(img['image_normal'], f / f.sum())
                img['image_seg'] = upfirdn2d.filter2d(img['image_seg'], f / f.sum())
        ...
        for key in logits:
            if 'normal' in key:
                ...
                logits['normal_face'] = logits['normal_face']*self.lambda_normal
        # logits['face'] = logits['face']*0.05
        # logits['full'] = logits['full']*0.0005
        # logits['lr'] = logits['lr']*0.05
        # logits['sr'] = logits['sr']*0.05
        return logits

    def accumulate_gradients(self, phase, real_img, real_seg, real_normal, real_c, gen_z, gen_c, gain, cur_nimg):
        ...
        neural_rendering_resolution = self.neural_rendering_resolution_initial//2
        ...
        # real_normal = filtered_resizing(real_normal, size=neural_rendering_resolution, f=self.resample_filter, filter_mode=self.filter_mode)

        real_img = {'image': real_img,
                    'image_raw': real_img_raw,
                    'image_seg': real_seg, 
                    'image_normal': real_normal, 
                    'image_face': crop_face(real_img, real_c, size=real_img.shape[-2]//8),
                    'normal_face': crop_face(real_normal, real_c, size=real_normal.shape[-2]//8),
                    }
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                loss_upsample_seg = torch.nn.functional.l1_loss(gen_img['seg_raw'],filtered_resizing(gen_img['image_seg'], size=gen_img['seg_raw'].shape[-2], f='antialiased')).mean()*100.0
                # loss_upsample_img = torch.square(gen_img['image_raw'] - filtered_resizing(gen_img['image'], size=gen_img['image_raw'].shape[-2], f='antialiased')).mean()
                loss_upsample_img = torch.nn.functional.l1_loss(gen_img['image_raw'],filtered_resizing(gen_img['image'], size=gen_img['image_raw'].shape[-2], f='antialiased')).mean()*100.0
                # loss_upsample_img = torch.nn.functional.l1_loss(img_filter(gen_img['image_raw']),gen_img['image']).mean()*1.0
                loss_upsample_normal = torch.nn.functional.l1_loss(gen_img['normal_raw'],filtered_resizing(gen_img['image_normal'], size=gen_img['normal_raw'].shape[-2], f='antialiased')).mean()*100.0
                training_stats.report('Loss/G/upsample_img', loss_upsample_img)
                training_stats.report('Loss/G/upsample_seg', loss_upsample_seg)
                training_stats.report('Loss/G/upsample_normal', loss_upsample_normal)
                # loss_upsample = torch.square(gen_img['image_raw'] - F.adaptive_avg_pool2d(gen_img['image'],gen_img['image_raw'].shape[2:4])).mean()
                loss_G += loss_upsample_img
                loss_G += loss_upsample_seg
                loss_G += loss_upsample_normal
        ...
        if phase in ['Greg', 'Gboth'] and self.G.rendering_kwargs.get('density_reg', 0) > 0 and self.G.rendering_kwargs['reg_type'] == 'l1':
            ...
            if self.style_mixing_prob > 0:
                with torch.autograd.profiler.record_function('style_mixing'):
                    ...
                    cutoff = torch.where(torch.rand([], device=ws.device) < self.style_mixing_prob, cutoff, torch.full_like(cutoff, ws.shape[1]))
                    ws[:, cutoff:] = self.G.mapping(torch.randn_like(z), c, update_emas=False)[:, cutoff:]
            initial_coordinates = torch.rand((ws.shape[0], 64000, 3), device=ws.device) * 2 - 1
            ...
            tmp_out = self.G.sample_mixed(all_coordinates, torch.randn_like(all_coordinates), ws, update_emas=False)
            # delta_sdf = tmp_out['delta_sdf']
            # sdf_loss = torch.nn.functional.smooth_l1_loss(delta_sdf, torch.zeros_like(delta_sdf)).mean()*100
            sigma = tmp_out['sigma']
            ...
            # (TVloss.mul(gain)+sdf_loss).backward()
                ...
                if phase in ['Dreg', 'Dboth']:
                    if self.dual_discrimination:
                        with torch.autograd.profiler.record_function('r1_grads'), conv2d_gradfix.no_weight_gradients():
                            ...
                            # inputs = [real_img_tmp['image_raw']]
                            inputs = [real_img_tmp['image']]
                            # inputs.append(real_img_tmp['image_seg'])
                            if self.normal_gan:
