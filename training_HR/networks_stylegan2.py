...
class FullyConnectedLayer(torch.nn.Module):
    ...
    def forward(self, x, frozen = False):
        if frozen:
            w = self.weight.to(x.dtype).data * self.weight_gain
            b = self.bias.data
        else:
...
class SynthesisLayer(torch.nn.Module):
    def __init__(self,
        ...
        if use_noise:
            ...
            # self.register_buffer('noise_const', torch.nn.Parameter(torch.randn([resolution, resolution])))
...
class SynthesisBlock(torch.nn.Module):
    ...
    def forward(self, x, img, ws, force_fp32=False, fused_modconv=None, update_emas=False, SR_flag=False, **layer_kwargs):
...
class SemanticGenerator(torch.nn.Module):
    ...
    def synthesis(self, ws, update_emas, **synthesis_kwargs):
        ...
        for i in range(len(self.synthesis_net)):
            ...
            img, x = self.synthesis_net[i](branch_ws, update_emas=update_emas, **synthesis_kwargs)
            imgs.append(img.view(img.shape[0], 3, img.shape[1]//3,img.shape[2],img.shape[3]))
            xs.append(x)
        return torch.cat(imgs,2), xs
    def forward(self, z, c, truncation_psi=1, truncation_cutoff=None, update_emas=False, **synthesis_kwargs):
        ws = self.mapping(z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        img, xs = self.synthesis(ws, update_emas=update_emas, **synthesis_kwargs)
        return img, xs
